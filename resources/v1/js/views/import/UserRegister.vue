<template>
  ...
  onMounted(async () => {
    ...
    if(route.path == '/v1/edit-profile'){
      userEditURL.value = true; 
      await setUserProfileData();
    } else if(route.path == '/v1/register'){
  ...
  function handleSubmitBtn(){
    ...
    }else if(route.path.includes('/v1/admin/users/edit/')){
      console.log('isPenalty',isPenalty.value);
      delete profile.value.socialLogin;
      updateUser(profile.value,userId.value, profile.status);
    }else if(route.path.includes('/v1/admin/users/create')){
      delete profile.value.socialLogin;
