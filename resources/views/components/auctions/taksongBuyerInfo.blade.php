<div class="py-4 border-bottom p-2">
    ...
    <div class="row row-cols-2 gy-3 text-muted bg-gray-50 rounded p-2">
        ...
        <div class="col-9 text-dark fw-semibold" 
            x-text="auction?.bids?.find(bid => bid.id === auction.bid_id)?.user?.dealer?.name ?? '미정'"
            @click="openAddressBookModal"
            ></div>
        <div class="col-3">주민(법인)번호</div>
        <div class="col-9 text-dark fw-semibold" 
            x-text="auction?.bids?.find(bid => bid.id === auction.bid_id)?.user?.dealer?.corporation_registration_number ?? '미정'"
            @click="openAddressBookModal"
            ></div>
