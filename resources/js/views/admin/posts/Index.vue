<template>
    <div class="p-3 row justify-content-center my-2">
      ...
      <router-link :to="{ name: 'posts.create' }" class="border-red-write">
            ...
            <tbody>
              <tr v-for="post in posts" :key="post.id">
                ...
                <td class="px-6 py-4 text-sm">
                  <div v-for="category in post.categories" :key="category.id">{{ category }}</div>
                </td>
                <td class="px-6 py-4 text-sm text-overflow">{{ post.title }}</td>
                <td class="px-6 py-4 text-sm">
                  <router-link :to="{ name: 'posts.edit', params: { id: post.id } }" class="badge">
                    <div class="icon-edit-img"></div>
                  </router-link>
                  <a href="#" @click.prevent="deletePost(post.id)" class="ms-2 badge web_style">
