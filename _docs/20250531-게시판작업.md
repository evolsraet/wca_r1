...
Route::prefix('board/{boardId}')->name('board.')->group(function () {
    ...
    Route::get('/', function ($boardId) {
        ...
        $skin = $board->skin ?: 'default'; // 기본 스킨: default
        return view("v2.boards.{$skin}.list", compact('board'));
    })->name('list');
    ...
    Route::get('/view/{articleId}', function ($boardId, $articleId) {
        $board = Board::findOrFail($boardId);
        $skin = $board->skin ?: 'default'; // 기본 스킨: default
        return view("v2.boards.{$skin}.view", compact('board', 'articleId'));
    })->name('view');
    ...
    Route::get('/form/{articleId?}', function ($boardId, $articleId = null) {
        $board = Board::findOrFail($boardId);
        $skin = $board->skin ?: 'default'; // 기본 스킨: default
        return view("v2.boards.{$skin}.form", compact('board', 'articleId'));
    })->name('form');
});
...
Route::prefix('board/{boardId}/articles')->name('api.boards.articles.')->group(function () {
...
- 예:
@if(auth()->user()?->hasRole($board->write_permission))
